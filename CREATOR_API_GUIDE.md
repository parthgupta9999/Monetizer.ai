...
Set the variables in `.env` (see `.env.example`), for example `SUPABASE_MARQUES_URL` and `SUPABASE_MARQUES_KEY`. `creator_config.py` reads those at runtime.
...
Or set the variables listed in `.env.example` (recommended for deployment).
