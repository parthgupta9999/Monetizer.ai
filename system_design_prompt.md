...
- **TTS (Text-to-Speech)**: ElevenLabs via server (`ELEVENLABS_API_KEY` in `.env`; never commit keys)
...
Credentials live in `.env` (see `.env.example`: `SUPABASE_*_URL`, `SUPABASE_*_KEY`, optional table overrides). Example table names: `mkbhd_videos`, `jerry_videos`, `unbox_videos`.
...
5. Backend `/api/tts` calls ElevenLabs with the server-side key and returns audio
